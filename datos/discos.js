...
function obtenerSentenciaSelectTodos(){
    return "SELECT * FROM discos"
}

function obtenerSentenciaSelectEspecifico(id){
    return "SELECT * FROM discos WHERE titulo = '" + id + "'"
}

function obtenerSentenciaInsert(o){
    // retorna la consulta INSERT para el objeto u (discos)
    // se asume que el objeto es de tipo discos
    var ret = "INSERT INTO discos (titulo, anio, nombregenero, nombreartista) VALUES (";
    ret += "'" + o.titulo + "', ";
    ret += "'" + o.anio + "', ";
    ret += "'" + o.nombregenero + "', ";
    ret += "'" + o.nombreartista + "') returning *";
    return ret;
}
...
function obtenerSentenciaUpdate(o, id){
    ...
    for (var i = 0; i < oKeys.length; i++) {
        switch (oKeys[i]){
            ...
            case "anio":
                ...
                break;
            case "nombregenero":
                ret += "nombregenero = '" + o.nombregenero + "'";
                break;
            case "nombreartista":
                ret += "nombreartista = '" + o.nombreartista + "'";
                break;
            default:
                break;            
        }
    ...
    ret += "WHERE titulo = '" + id + "' returning *"
    return ret;
}

function obtenerSentenciaDelete(id){
    return "DELETE FROM discos WHERE titulo = '" + id + "' returning *"
}
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
